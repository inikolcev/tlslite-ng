...
class TLSRecordLayer(object):
    ...
    def __init__(self, sock):
        ...
        # function that will be called with one argument - the
        # CertificateRequest received from server
        # needs to return tuple with certificate chain and private key
        # (they can be None if no certificate matches server request)
        # TODO: move to HandshakeSettings
        self.pha_callback = None
    ...
    def _handle_pha(self, cert_request):
        cert, p_key = self.pha_callback(cert_request)
        ...
        if p_key:
            ...
            if not valid_sig_algs:
                for result in self._sendError(
                    ...
                    yield result
            avail_sig_algs = self._sigHashesToList(HandshakeSettings(), p_key, cert,
                                                   version=(3, 4))
